...
FLAML applies a fresh clone to each training partition and to final model training while leaving validation partitions unchanged. Automatic rare-class augmentation is disabled when `resampler` is set. Automatically generated classification holdout partitions remain disjoint; a class with only one example raises `ValueError` because it cannot appear in both partitions without duplication. `resampler` cannot be combined with non-null `sample_weight`, `groups`, or `ensemble`. `sample_weight=None` is accepted, including in estimator-specific settings. Pass the same sampler to `retrain_from_log()` when replaying a selected configuration.
