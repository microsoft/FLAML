...
parser.add_argument("--k", dest="k", help="k", default=2, type=int)
args = parser.parse_args()
args.folder = args.folder + "_baseline_fewshot" "_t" + str(args.temperature) + "_seed" + str(args.seed)
...
def fewshot_solve(model, problem, problem_set, max_tokens=None, k =2):
    # few shot examplars
    examplars = random_select(problem_set, problem, k = k)
    few_shot_prompt = few_shot_template(examplars)
    full_prompt = few_shot_prompt + """Solve a math problem carefully. Put the final answer in \\boxed{}.\n\nProblem: """
    ...
    for problem_set in problem_sets:  # one problem_set is one category
        ...
        for count, problem in enumerate(problem_set):
            ...
            if int(problem["problem_id"]) in done_problems:
                ...
                continue
            results = fewshot_solve(engine, problem, problem_set, max_tokens=None, k =args.k)
            if results is None:
                break
